...
async fn proxy_request(app: &Arc<App>, req: Request) -> Response {
    ...
    // to reuse under the same policy (deterministic, opted in, or attested by
    // a verifier oracle here), and not replay, whose whole point is zero
    // network. The attestation is read from this instance's markers, so a
    // sibling's oracle verdict does not travel with its bytes.
    if !app.opts.peers.is_empty()
        && !app.opts.replay
        && app.opts.cache
        && reusable(&body, &headers, &app.attested_dir, &req_key)
    {
